...
- [x] 準本番環境の自動デプロイとデータ同期 (CI/CD 拡張)
    - [x] GitHub Actions による `staging` ブランチの自動デプロイ設定（低コストモデル `gemini-1.5-flash` をデフォルトに設定）
    - [x] 匿名化機能付きデータ同期スクリプト (`tools/sync_prod_to_staging.py`) の開発（本番DBの保護とデータ分離を徹底）
    - [x] ステージング環境での動作検証と品質確認
- [x] マネーフォワード等の連携を通じたデータの自動取得 (2025-02-17)
- [x] AIによる基本的な週次・月次レビューの生成とオンデマンド実行 (2025-02-17)
- [x] Webダッシュボードへの統合（データ取得・分析実行UI） (2025-02-17)
- Slack通知の更なる高度化
...
- [ ] Web UI からの CSV 手動インポート機能 (Backend API & Frontend)
- [ ] AI キャラクターのパーソナライズ（設定画面での切り替え）

### Phase 3: ユーザー体験とインフラの向上
- [ ] CI/CD パイプラインの完全統合（Staging 自動デプロイとテスト）
...
- [x] **ログ出力の標準化**: `src/` 全体のログ出力を標準化し、`docs/TROUBLESHOOTING.md` との連携を強化。 (2025-02-18)
- [ ] **Tools ディレクトリの整理と統合 CLI の開発 (Issue #106)**: `tools/` 内のスクリプトを機能別に整理し、一貫したコマンドラインインターフェースを提供。 (Task 4 - **InProgress**)
