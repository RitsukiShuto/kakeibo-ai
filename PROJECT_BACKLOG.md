...
- [x] 本番環境でのAI分析失敗の調査と復旧 (Issue #114) (2026-05-21)
...
- **AI分析エンジンの責務分割とリファクタリング (Issue #6)**: `KakeiboAnalyzer` を機能（定期レビュー、チャット、ライフプラン等）ごとに分割し、LLMコンテキスト消費を削減し保守性を向上。
- **トークン消費削減のための明細要約ロジック導入 (Issue #7)**: APIコスト削減のため、金額上位抽出や集計データ化を行う前処理ロジックを実装。
- **フロントエンドのユニットテスト環境構築 (Issue #8)**: `Vitest` と `React Testing Library` を導入し、UIコンポーネントのテストをCIに統合。
