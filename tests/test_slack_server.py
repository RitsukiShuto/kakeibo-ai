...
def test_handle_review_command():
    ...
    with patch('src.output.slack_server.threading.Thread') as mock_thread:
        # handle_review_command(ack, command, respond)
        handle_review_command(mock_ack, command, mock_respond)
        
        mock_ack.assert_called_once()
        mock_respond.assert_called_once()
        assert "monthly" in mock_respond.call_args[0][0].lower()
...
def test_handle_review_command_others():
    ...
    with patch('src.output.slack_server.threading.Thread'):
        ...
        assert "yearly" in mock_respond.call_args[0][0].lower()
        
        mock_respond.reset_mock()
        handle_review_command(mock_ack, {"user_id": "U", "text": "q"}, mock_respond)
        assert "quarterly" in mock_respond.call_args[0][0].lower()
        
        mock_respond.reset_mock()
        handle_review_command(mock_ack, {"user_id": "U", "text": ""}, mock_respond)
        assert "weekly" in mock_respond.call_args[0][0].lower()
...
def test_start_slack_server(mock_handler):
    ...
    with patch('src.output.slack_server.SLACK_APP_TOKEN', 'xapp-123'):
        with patch('src.output.slack_server.SLACK_BOT_TOKEN', 'xoxb-123'):
            start_slack_server()
            mock_instance.start.assert_called_once()

def test_start_slack_server_no_token():
    # トークンが正しく設定されていない場合に起動しないことを確認
    with patch('src.output.slack_server.SLACK_APP_TOKEN', None):
        start_slack_server()
    with patch('src.output.slack_server.SLACK_APP_TOKEN', ''):
        start_slack_server()
    with patch('src.output.slack_server.SLACK_BOT_TOKEN', 'xoxb-dummy'):
        start_slack_server()
