...
- [ ] 準本番環境の自動デプロイとデータ同期 (CI/CD 拡張)
    - [ ] GitHub Actions による `staging` ブランチの自動デプロイ設定（低コストモデル `gemini-1.5-flash` をデフォルトに設定）
    - [ ] 匿名化機能付きデータ同期スクリプト (`tools/sync_prod_to_staging.py`) の開発（本番DBの保護とデータ分離を徹底）
    - [ ] ステージング環境での動作検証と品質確認
