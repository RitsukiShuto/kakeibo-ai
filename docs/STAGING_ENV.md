...
| :--- | :--- | :--- | :--- |
| **プロジェクト名** | `kakeibo-ai` (デフォルト) | `STG_kakeibo-ai` | `-p STG_kakeibo-ai` で指定 |
| **Frontend ポート** | `5173` | `5174` | `http://[IP]:5174` でアクセス |
| **Backend ポート** | `8000` | `8001` | |
| **コンテナ名 prefix** | `kakeibo-*` | `STG_kakeibo-ai-*` | |
...
### C. デプロイ検証
`staging` ブランチにプッシュすると、GitHub Actions により Raspberry Pi の `/home/r410/STG_kakeibo-ai` ディレクトリに自動デプロイされます。

デプロイ後は以下のコマンドで状態を確認できます（Raspberry Pi上）：
```bash
cd /home/r410/STG_kakeibo-ai
...
- デプロイ先: `/home/r410/STG_kakeibo-ai`
